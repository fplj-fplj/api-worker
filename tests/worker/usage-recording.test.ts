						...
						async run() {
							if (sql.startsWith("INSERT INTO usage_logs")) {
								rows.push({
									...
									request_entry_format: params[8],
									total_tokens: params[9],
									prompt_tokens: params[10],
									completion_tokens: params[11],
									cost: params[12],
									cache_read_input_tokens: params[13],
									cache_write_input_tokens: params[14],
									uncached_input_tokens: params[15],
									billable_input_tokens: params[16],
									charge_amount: params[17],
									charge_currency: params[18],
									charge_status: params[19],
									charge_source: params[20],
									latency_ms: params[22],
									status: params[26],
...
describe("usage recording", () => {
	...
	it("记录缓存 token 与下游计费字段", async () => {
		...
		await recordUsage(db as never, {
			...
			model: "gpt-4o-mini",
			requestPath: "/v1/chat/completions",
			requestEntryFormat: "openai_chat",
			totalTokens: 4000,
			promptTokens: 3000,
